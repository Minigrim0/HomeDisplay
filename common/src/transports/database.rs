...
fn store_departures(new_departures: &Vec<Departure>, site_id: &str, redis_data: &settings::Redis) -> Result<(), String> {
...
pub async fn fetch_new_sites(stops: &Vec<settings::BusStop>, redis_data: &settings::Redis) -> Result<Vec<Site>, String> {
    ...
    for site in sites.iter() {
        if stops.is_empty() || stops.iter().any(|s| unidecode(&site.name.to_lowercase()).contains(&unidecode(&s.name.to_lowercase())) && s.site_id.as_ref().and_then(|id| Some(id == &site.id)) != Some(false)) {
...
pub async fn get_sites(stops: &Vec<settings::BusStop>, redis_data: &settings::Redis) -> Result<Vec<Site>, String> {
    ...
    for site in sites.iter() {
        if let Ok(serialized_site) = database::get_redis_key(site.to_string(), redis_data).await {
            ...
            if stops.is_empty() || stops.iter().any(|stop| unidecode(site_name).contains(&unidecode(&stop.name.to_lowercase())) && stop.site_id.as_ref().and_then(|id| Some(id == &site.id)) != Some(false)) {
                site_list.push(site);
            }
        } else {
...
pub async fn get_departures(site_id: String, redis_data: &settings::Redis) -> Result<Vec<Departure>, String> {
    match database::get_redis_key(format!("homedisplay:sites:{}:departures", site_id), redis_data).await {
