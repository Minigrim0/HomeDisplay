...
pub async fn fetch_conversion() -> Option<Conversion> {
    let api_key: String = var("OER_API_KEY").expect("OER_API_KEY is required to run this hook").to_string();

    let from_currency: String = var("OER_FROM").unwrap_or(
        {
            println!("Using default latitude value (Err: Missing OER_FROM)");
            "EUR".to_string()
        }
    );
    ...
    let to_currency: String = var("OER_TO").unwrap_or(
        {
            println!("Using default latitude value (Err: Missing OER_TO)");
            "SEK".to_string()
        }
    );
